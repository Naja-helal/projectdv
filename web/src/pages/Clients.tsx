...
import { clientsApi, projectsApi } from '@/lib/supabaseApi';
...
export default function ClientsPage() {
  ...
  // جلب المشاريع لحساب الإحصائيات
  const { data: projects = [] } = useQuery({
    queryKey: ['projects'],
    queryFn: projectsApi.getAll,
  });

  // ربط العملاء بمشاريعهم وحساب الإحصائيات
                  ...
                  <div className="flex-1 w-full" onClick={() => handleView(client)}>
                    <div className="flex flex-wrap items-center gap-2 mb-2">
                      <span className="text-2xl">
                        {typeof client.icon === 'object' && client.icon !== null 
                          ? ((client.icon as any).symbol || (client.icon as any).name || '👤') 
                          : (client.icon || '👤')}
                      </span>
            ...
            <AlertDialogDescription>
              ...
              {selectedClient?.projects_count && selectedClient.projects_count > 0 && (
                <span className="block mt-2 text-amber-600 font-semibold">
                  ℹ️ ملاحظة: هذا العميل لديه {selectedClient.projects_count} مشروع مرتبط. 
                  يمكنك حذف العميل وستبقى المشاريع بدون عميل.
                </span>
              )}
            </AlertDialogDescription>
          </AlertDialogHeader>
