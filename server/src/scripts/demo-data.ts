...
const finalStats = {
  ...
  customValues: db.prepare("SELECT COUNT(*) as count FROM custom_values").get() as { count: number },
  totalAmount: db.prepare("SELECT SUM(total_amount) as total FROM expenses").get() as { total: number | null }
};
